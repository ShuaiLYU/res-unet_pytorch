...
class Res18_UNet(UNet):
    def __init__(self,n_classes,norm_layer=None,bilinear=True):
        self.base_channels = 32   # resnet18 和resnet34 这里为 32 , 64
        level = 5
        padding = 1
        super(Res18_UNet,self).__init__(n_classes, self.base_channels,level,padding,norm_layer,bilinear)
    def build_encoder(self):
        return _resnet('resnet18', BasicBlock, [2, 2, 2, 2],base_planes= self.base_channels )
...
if __name__=="__main__":
    ...
    res18net=Res18_UNet(n_classes=10)
    opt=res18net(ipt)
    print(opt.shape)
